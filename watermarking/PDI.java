...
//	private static LookupTable HEIGHT_TABLE = new LookupTable();
	...
	//TODO
//	public static void patchwork(double[][] image, int delta, int markSize){
//	public static int[][] patchwork(double[][] image, int delta, int markSize){
//		//double[][] ret = new double[image.length][image[0].length];
//		int max = image.length * image[0].length;
//		//int setSize = 4;
//		boolean[] visitedPixels = new boolean[max];
//		double[] singleDimensionImage = vetorizeImage(image);
//		int[][] subsets = new int[2][markSize];
//		int subsetA = 0;
//		int subsetB = 1;
//		subsets[subsetA] = createSubSet(max, visitedPixels, markSize);
//		subsets[subsetB]= createSubSet(max, visitedPixels, markSize);
//		
//		double sum1 = sumSet(subsets[subsetA], subsets[subsetB], singleDimensionImage);
//		//sumSet(subsetA, subsetB, singleDimensionImage);
//		for (int i = 0; i < subsets[subsetA].length; i++) {
//			singleDimensionImage[subsets[subsetA][i]] += delta;
//			singleDimensionImage[subsets[subsetB][i]] -= delta;
//		}
//		
//		double sum2 = sumSet(subsets[subsetA], subsets[subsetB], singleDimensionImage);
//		double[][] ret = unvetorizeImage(singleDimensionImage, image.length, image[0]. length);
//		
//		System.out.println("result: " + (sum2 - sum1));
//		System.out.println("2 * d * n = " + (2* delta * markSize));
//		
//		
//		return subsets;
//	}
	...
	public static double[] vetorizeImage(double[][] image){
		double[] ret = new double[image.length * image[0].length];
		int index = 0;
	...
	public static double[][] unvetorizeImage(double[] image, int height, int width){
...
//				currentIndex++;
	...
	public static int[] createSubSet(int max, boolean[] pixels, int setSize){
		Random r = new Random();
	...
	public static void calculateS(double[][] image, double[][] copy){
		double sum = 0; 
		
		for (int i = 0; i < image.length; i++) {
			for (int j = 0; j < image[0].length; j++) {
				sum += image[i][j] - copy[i][j];
//				if(image[i][j] != copy[i][j]){
...
//	}
	...
	public static void applyDelta(int[] setA, int[] setB, int delta){
